...
#parameters of the 4-layer network
decay = 1e-6
batch_size = 32
...
w1, b1 = init_weights(36, 10), init_bias(10) #weights and biases from input to first hidden layer
w2, b2 = init_weights(10, 10), init_bias(10) #weights and biases from first hidden layer to second hidden layer
wo, bo = init_weights(10, 6, logistic=False), init_bias(6) #weights and biases from second hidden to output layer

h1 = T.nnet.sigmoid(T.dot(X, w1) + b1)
h2 = T.nnet.sigmoid(T.dot(h1, w2) + b2)
py = T.nnet.softmax(T.dot(h2, wo) + bo)

y_x = T.argmax(py, axis=1)

cost = T.mean(T.nnet.categorical_crossentropy(py, Y)) + decay*(T.sum(T.sqr(w1)+T.sum(T.sqr(w2)+T.sum(T.sqr(w2)))))
...
plt.tight_layout()
plt.savefig('sample_cost_and_accuracy_4_layers_1000_epochs.png')
